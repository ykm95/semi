...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnrsearch").click(function() {
			location.href = "/adminreport/list?search=" + $("#search2").val();
		...
		<table class="table table-hover table-condensed">
			...
			<c:forEach items="${ rlist }" var="i">
				<tr>
					...
					<td>${i.userno }</td>
					<td>${i.userid }</td>
					<td><a href="/adminreport/view?userno=${i.userno}">${i.repotitle }</a></td>
				</tr>
			</c:forEach>
		</table>
		<jsp:include page="/WEB-INF/views/admin/report/report_paging.jsp" />

		<div id="search1">
			<input type="text" id="search2">
			<button id="btnrsearch">검색</button>
		</div>
		</div>
		</div>
	</div>
