...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnsearch").click(function() {
			location.href = "/admin/userlist?search=" + $("#search2").val();
		});

		//회원 탈퇴
		$(".btndelete").click(function() {

			if (confirm('정말 회원을 탈퇴할까요?ㅠㅠ')) {
				$(location).attr("href", "/user/delete?userno=" + $(this).data("userno"));
			}
		})
		...
		<table class="table table-hover table-condensed">
			...
			<c:forEach items="${ list }" var="i">
				<tr style="text-align: center; height: 30">
					...
					<td>${ i.email }</td>
					<td>${ i.nick }</td>
					<td><a href="/adminrecipe/userRecipelist?userno=${i.userno }"><button class="btn">정보확인</button></a></td>
					<td><a href="/admin/notice?userno=${i.userno }"><button class="btn" style="background-color:#F2F5A9">알림전송</button></a></td>
					<td><button	id="btndelete" class="btn btndelete" data-userno="${i.userno }" style="background-color:#FA5858">회원탈퇴</button></td>
				</tr>
			</c:forEach>
		</table>
		<jsp:include page="/WEB-INF/views/admin/user/user_paging.jsp" />
		<div id="seach1">
